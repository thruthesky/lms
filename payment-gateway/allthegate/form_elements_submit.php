...
$options = serialize( $_REQUEST );

// min
if ( empty($_REQUEST['min']) ) return jsAlert("수업 분을 선택하십시오.");
$min = $_REQUEST['min'];
...
// payment method
if ( empty($_REQUEST['method']) ) return jsAlert("결재 방식을 선택하십시오.");
$method = $_REQUEST['method'];


// insert db
$q = "INSERT INTO $table (user_id, method, amount, stamp_create, options) VALUES ( %d, %s, %d, %d, %s )";
$re = $wpdb->query( $wpdb->prepare( $q, $user_id, $method, $amt, time(), $options ) );

if ( $re === false ) {
    return jsAlert("Error on inserting payment information");
}
$OrdNo = $wpdb->insert_id;
